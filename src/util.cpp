...
const char * const BITCOIN_CONF_FILENAME = "ultrablock.conf";
const char * const BITCOIN_PID_FILENAME = "ultrablock.pid";
...
#else
    const char* pszModule = "ultrablock";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Ultrablock";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Ultrablock";
#else
    // Unix
    return pathRet / ".ultrablock";
#endif
#endif
}
