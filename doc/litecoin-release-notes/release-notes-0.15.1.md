Ultrablock Core version *0.15.1* is now available from:

  <https://download.ultrablock.org/ultrablock-0.15.1/>
...
A number of changes to the way Ultrablock Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 blessings per kB (0.00001 UBC). This is to prepare for dropping the minimum transaction fee to 0.1 blessings per kB (0.0001 UBC) in the next release.
